...
```mermaid
graph TD
    A[REACT COMPONENTS<br/>component-library/pages] -->|fetch| B[API ROUTES<br/>app/api/feature/*]
    B -->|controller.execute| C[CONTROLLER<br/>Infrastructure<br/>Maps HTTP params → UseCase request]
    C -->|useCase.execute| D[USE CASE<br/>Core - Business Logic<br/>Validates → Calls Gateway → Processes → Presents]
    D -->|gateway.list| E[GATEWAY<br/>Infrastructure<br/>Implements Port → Delegates to Endpoints]
    E -->|endpoint.fetch| F[ENDPOINT<br/>Infrastructure<br/>Builds HTTP request → Calls Rucio Server]
